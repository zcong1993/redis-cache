...
const rc = new RedisCache({ client, nonExistsExpire: 5 })
...
const main = async () => {
  const res = await Promise.all(
    Array(10)
      .fill(null)
      .map(() => rc.batchGetArray('test', arrFn, ['test1', 'test2'], 'k', 10))
  )
  console.log(res, rc.stats)

  const res1 = await rc.getOne('single', singleFn, 'test', 10)
  console.log(res1, rc.stats)

  const res2 = await rc.cacheFn('fn-test', fn, 10)
  console.log(res2, rc.stats)

  const res3 = await Promise.all(
    Array(10)
      .fill(null)
      .map(() => rc.batchGet('test-map', mapFn, ['test1', 'test2'], 10))
  )
